...
// curl http://localhost:3000/api/messages/xxx
router.get('/:messageId', (req, res) => {
  return res.send(req.context.models.messages[req.params.messageId]);
});
// curl -X POST -H "Content-Type:application/json" http://localhost:3000/api/messages -d '{"text":"Hi again, World"}'
router.post('/', (req, res) => {
  const id = uuidv4();
  const message = {
    id,
...
// curl -X PUT -H "Content-Type:application/json" http://localhost:3000/api/messages/1 -d '{"text":"Hi, Herman"}'
